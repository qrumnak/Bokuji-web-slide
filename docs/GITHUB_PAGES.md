...
公開対象はリポジトリ直下ではなく、GitHub ActionsがViteで生成した`dist`です。ワークフローはPagesの公開元をGitHub Actionsに設定し、アップロード前に公開用`index.html`が`/src/main.jsx`ではなく`/Bokuji-web-slide/assets/`配下のハッシュ付きJavaScriptを参照していることを検証します。
